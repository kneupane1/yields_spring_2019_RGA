...
size_t run(std::shared_ptr<TChain> _chain, const std::shared_ptr<SyncFile>& _sync, int thread_id) {
  ...
  for (size_t current_event = 0; current_event < num_of_events; current_event++) {
    ...
    // for (int layer_count = 0; layer_count < data->sc_extras_size(part); layer_count++) {
    //   // if ((data->pid(part) == 0)){
    //     // && (data->charge(part) > 0) && (data->sc_extras_size(part) == 3)) {
    //     csv_data output;

    //     std::cout << "  extra size  " << data->sc_extras_size(part) << std::endl;
    //     std::cout << "  pindex  " << data->sc_cnd_pindex(part) << std::endl;
    ...
    //     output.cnd_pindex = data->sc_cnd_pindex(part);
